...
class WasmChessGame {
	...
	makePromotionMove(fromRow, fromCol, toRow, toCol, promotionPiece) {
		if (!this.game) return false;
		return this.game.make_promotion_move(
			fromRow,
			fromCol,
			toRow,
			toCol,
	...
	isPromotionMove(fromRow, fromCol, toRow, toCol) {
		if (!this.game) return false;
		return this.game.is_promotion_move(fromRow, fromCol, toRow, toCol);
	}
...
async function initializeGame() {
	...
	if (initialized) {
		...
		loadingMessage.textContent =
			"Failed to load WASM module. Please check console for errors.";
...
function updateDisplay() {
	...
	squares.forEach((square, index) => {
		...
		if (
			chessGame.validMoves.some(
				(move) => move.to_row === displayRow && move.to_col === displayCol
			)
		) {
	...
	document.getElementById("current-turn").textContent =
		gameState.current_player.charAt(0).toUpperCase() +
		gameState.current_player.slice(1);
}
...
function handleSquareClick(event) {
	...
	if (
		chessGame.selectedSquare &&
		chessGame.validMoves.some(
			(move) => move.to_row === actualRow && move.to_col === actualCol
		)
	) {
		// Check if this is a promotion move
		if (
			chessGame.isPromotionMove(
		...
		if (success) {
			...
			const fromNotation =
				String.fromCharCode(97 + chessGame.selectedSquare.col) +
				(8 - chessGame.selectedSquare.row);
	...
	if (piece) {
		const pieceColor = piece.toLowerCase().includes("white")
			? "white"
			: "black";
...
function addMoveToHistory(moveNotation) {
	...
	moveElement.textContent = `${Math.ceil(
...
function setupEventListeners() {
	...
	document
		.getElementById("cancel-fen-btn")
		.addEventListener("click", cancelFenInput);
...
function showPromotionDialog(fromRow, fromCol, toRow, toCol) {
	...
	// Update piece colors in the dialog based on current player
	const isWhite = gameState.current_player === "white";
	const pieces = modal.querySelectorAll(".promotion-piece");

	pieces[0].textContent = isWhite ? "♕" : "♛"; // Queen
	pieces[1].textContent = isWhite ? "♖" : "♜"; // Rook
	pieces[2].textContent = isWhite ? "♗" : "♝"; // Bishop
	pieces[3].textContent = isWhite ? "♘" : "♞"; // Knight
	...
	promotionButtons.forEach((btn) => {
		btn.onclick = () => {
			...
			handlePromotion(fromRow, fromCol, toRow, toCol, piece);
		};
	});
...
function handlePromotion(fromRow, fromCol, toRow, toCol, promotionPiece) {
	const modal = document.getElementById("promotion-modal");
	modal.style.display = "none";

	// Make the promotion move
	const success = chessGame.makePromotionMove(
		fromRow,
		fromCol,
		toRow,
		toCol,
	...
	if (success) {
		// Record move in history with promotion notation
		const fromNotation = String.fromCharCode(97 + fromCol) + (8 - fromRow);
		const toNotation = String.fromCharCode(97 + toCol) + (8 - toRow);
		const promotionSymbol = promotionPiece.charAt(0).toUpperCase();
		addMoveToHistory(`${fromNotation} → ${toNotation}=${promotionSymbol}`);

		chessGame.clearSelection();
		updateDisplay();
		updateGameStatus();
	} else {
		// If the move failed, show an error and clear selection
